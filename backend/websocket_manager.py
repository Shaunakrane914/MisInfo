...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        if websocket in self.active_connections:
            ...
            logger.warning("Attempted to remove WebSocket connection that was not in active connections list")
    ...
    async def broadcast(self, message: dict):
        ...
        # Keep track of dead connections to remove later
        dead_connections = []
        ...
        for connection in self.active_connections:
            try:
                ...
                logger.debug("Message sent to WebSocket connection")
            except Exception as e:
                # If an exception occurs, the connection is dead
                logger.warning(f"Failed to send message to WebSocket connection: {e}")
                # Add the dead connection to the list
                dead_connections.append(connection)
        
        # Remove dead connections from the active connections list
